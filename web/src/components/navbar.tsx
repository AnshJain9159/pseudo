  ...
  const Navbar: React.FC = () => {
    ...
    const NavDropdown: React.FC<{ category: string; items: NavItem[] }> = ({ category, items }) => (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="px-4 py-2 text-sm font-medium text-gray-300 hover:text-white">
            {category} <ChevronDown className="ml-1 h-4 w-4" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent className="bg-gray-900 border-gray-700">
          {items.map((item) => (
            <DropdownMenuItem key={item.name} className="hover:bg-gray-800">
              <NavLink item={item} />
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
    ...
    const AuthButton: React.FC<{ href: string; onClick?: () => void; children: React.ReactNode }> = ({ href, onClick, children }) => (
      <Link
        href={href}
        onClick={onClick}
        className="px-4 py-2 text-sm font-medium text-white bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors duration-300"
      >
        {children}
      </Link>
    );
    ...
    return (
      ...
      <div className="container mx-auto px-6 py-4">
        ...
        {isSidebarOpen && (
          ...
          <div
            className="fixed right-0 top-0 h-full w-64 bg-gray-900 shadow-lg z-50 overflow-y-auto"
